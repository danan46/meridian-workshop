      ...
      <div v-if="isOpen && backlogItem" class="modal-overlay" @click="close">
        <div class="modal-container" @click.stop>
          <div class="modal-header">
            <h3 class="modal-title">{{ t('modal.backlogDetail.title') }}</h3>
          ...
          <div class="modal-body">
            <div class="shortage-header">
              ...
              <span class="priority-badge" :class="backlogItem.priority">
                {{ backlogItem.priority }} {{ t('modal.backlogDetail.prioritySuffix') }}
              </span>
            </div>

            <div class="shortage-summary">
              <div class="summary-card danger">
                <div class="summary-label">{{ t('modal.backlogDetail.shortageAmount') }}</div>
                <div class="summary-value">{{ shortage }} units</div>
              </div>
              <div class="summary-card warning">
                <div class="summary-label">{{ t('modal.backlogDetail.daysDelayed') }}</div>
            ...
            <div class="info-grid">
              <div class="info-item">
                <div class="info-label">{{ t('modal.backlogDetail.orderId') }}</div>
                <div class="info-value order-id">{{ backlogItem.order_id }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.backlogDetail.itemSku') }}</div>
                <div class="info-value sku">{{ backlogItem.item_sku }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.backlogDetail.quantityNeeded') }}</div>
                <div class="info-value">{{ backlogItem.quantity_needed }} units</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.backlogDetail.quantityAvailable') }}</div>
                <div class="info-value">{{ backlogItem.quantity_available }} units</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.backlogDetail.expectedDate') }}</div>
                <div class="info-value">{{ formatDate(backlogItem.expected_date) }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.backlogDetail.status') }}</div>
                <div class="info-value">
                  <span class="badge danger">{{ t('modal.backlogDetail.backordered') }}</span>
                </div>
              </div>
            </div>
          </div>
