      ...
      <div v-if="isOpen && inventoryItem" class="modal-overlay" @click="close">
        <div class="modal-container" @click.stop>
          <div class="modal-header">
            <h3 class="modal-title">{{ t('modal.inventoryDetail.title') }}</h3>
          ...
          <div class="modal-body">
            ...
            <div class="stock-summary">
              <div class="summary-card primary">
                <div class="summary-label">{{ t('modal.inventoryDetail.quantityOnHand') }}</div>
                <div class="summary-value">{{ inventoryItem.quantity_on_hand }} units</div>
              </div>
              <div class="summary-card" :class="getSummaryCardClass()">
                <div class="summary-label">{{ t('modal.inventoryDetail.stockLevel') }}</div>
                <div class="summary-value">{{ stockPercentage }}%</div>
                <div class="summary-subtitle">{{ t('modal.inventoryDetail.vsReorderPoint') }}</div>
              </div>
            </div>

            <div class="info-grid">
              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.category') }}</div>
                <div class="info-value">{{ inventoryItem.category }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.location') }}</div>
                <div class="info-value">{{ inventoryItem.location }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.reorderPoint') }}</div>
                <div class="info-value">{{ inventoryItem.reorder_point }} units</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.unitsRemaining') }}</div>
              ...
              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.unitCost') }}</div>
                <div class="info-value">{{ currencySymbol }}{{ inventoryItem.unit_cost.toFixed(2) }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.totalValue') }}</div>
              ...
              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.warehouse') }}</div>
                <div class="info-value">{{ translateWarehouse(inventoryItem.location) }}</div>
              </div>

              <div class="info-item">
                <div class="info-label">{{ t('modal.inventoryDetail.status') }}</div>
                <div class="info-value">
          ...
          <div class="modal-footer">
            <button class="btn-secondary" @click="close">{{ t('modal.close') }}</button>
          </div>
        </div>
      </div>
    </Transition>
